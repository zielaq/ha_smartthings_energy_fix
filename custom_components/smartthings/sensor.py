                        ...
                        for description in descriptions:
                            if (
                                ...
                                if (
                                    ...
                                    continue
                                sensor_cls = (
                                    SmartThingsAccumulatingEnergySensor
                                    if description.key == "energy_meter"
...
class SmartThingsAccumulatingEnergySensor(SmartThingsSensor):
    """Energy sensor that accumulates deltaEnergy for devices where energy is always 0.

    Samsung TVs report energy=0 in powerConsumption, with actual data in
    deltaEnergy (per-window Wh). This sensor detects that and accumulates
    deltas into a running total compatible with state_class TOTAL_INCREASING.
    For devices with working energy field (appliances), it passes through directly.
    """
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._uses_delta: bool | None = None
    ...
    def native_value(self) -> float | None:
        ...
        if not isinstance(energy, (int, float)):
            energy = 0
        delta_energy = res.get("deltaEnergy", 0)
        if not isinstance(delta_energy, (int, float)):
            delta_energy = 0

        # Decide mode on first meaningful data.
        if self._uses_delta is None and (energy > 0 or delta_energy > 0):
            self._uses_delta = energy == 0 and delta_energy > 0

        # Standard mode: device reports cumulative energy (appliances).
        if not self._uses_delta:
            return energy / 1000 if energy > 0 else 0.0

        # Delta mode: accumulate deltaEnergy (TVs).
        # Use the report "end" timestamp to avoid double-counting the same window.
        end = res.get("end")
